...
function getConfiguredPlaceholder(config, colNumber, fallback = '') {
    // Custom hints retired; retain built-in date/year format hints only.
    return fallback;
}
...
let autocompleteGlobalClickListenerAdded = false;

function setupGlobalAutocompleteListener() {
    if (autocompleteGlobalClickListenerAdded) return;
    document.addEventListener("click", function(e) {
        let lists = document.getElementsByClassName("autocomplete-items");
        for (let i = 0; i < lists.length; i++) {
            const inputId = lists[i].id.replace("autocomplete-list", "");
            const inp = document.getElementById(inputId);

            if (lists[i].contains(e.target) || e.target === inp) {
                return; // Clicked inside the list or on the input itself
            }
        }

        // If clicked outside, close all lists
        let x = document.getElementsByClassName("autocomplete-items");
        for (let i = x.length - 1; i >= 0; i--) {
            x[i].parentNode.removeChild(x[i]);
        }
    });
    autocompleteGlobalClickListenerAdded = true;
}
...
function autocomplete(inp, arr, extractMode = "none") {
    ...
    // Close when clicking outside (handled globally to prevent memory leaks)
    setupGlobalAutocompleteListener();
