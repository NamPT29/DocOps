...
async function toggleCheckSubmission(id, checkbox) {
    try {
        const response = await authFetch(`/api/submissions/${id}/toggle_check`, { method: 'PUT' });
        const res = await response.json();
        if (res.status === 'ok') {
...
async function copySubmission(id) {
    ...
    try {
        const response = await authFetch(`/api/submissions/${id}/copy`, { method: 'POST' });
        const res = await response.json();
        if (res.status === 'ok') {
            fetchSubmissions();
            if (res.new_id) {
            ...
            if (currentUser && currentUser.role === 'admin') {
                ...
                document.querySelectorAll('.error-checkbox').forEach(cb => {
                    cb.onchange = async () => {
                        ...
                        try {
                            const response = await authFetch(`/api/submissions/${id}/errors`, {
                                method: 'PUT',
                            ...
                            const res = await response.json();
                            if (res.status !== 'ok') {
                                alert('Lỗi khi lưu trạng thái sai: ' + res.message);
                            }
                        } catch(e) {
                            alert('Lỗi kết nối khi lưu trạng thái sai');
                        }
                    };
                });
            } else {
...
async function deleteSubmission(id) {
    ...
    try {
        const response = await authFetch(`/api/submissions/${id}`, { method: 'DELETE' });
        const res = await response.json();
...
async function fetchDocumentPool() {
    try {
        const res = await authFetch('/api/documents/pool');
...
async function assignDocuments() {
    ...
    try {
        const res = await authFetch('/api/documents/assign', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
